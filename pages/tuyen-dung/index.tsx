...
function Recruit() {
    ...
    const getRecruits = async () => {
        try {
            setIsLoading(true);
            const result = await dispatch(getRecruitsByDomain({
                pageSize: 7,
                pageIndex: 1,
                status: 1,
                domain: "gofiber.vn"
                        ...
                        <div className='row'>
                            {
                                recruits.map((item, index) => (
                                    <RecruitItem item={item} wrapperClassName={`col ${index === 0 ? 'col-12 primary' : 'col-12 col-sm-6 col-lg-4 mt-4 secondary'}`} key={index} />
                                ))
                            }
                        </div>
                        <div className='row'>
                            <div className='mt-4 d-flex justify-content-center'>
                                <button type="button" className={`btn0 button_text_icon button_text_icon__white mt-4`}>
                                    <span>Xem thêm</span>
                                    <Image style={{ marginLeft: '8px' }} src={require("@/public/images/icons/right.svg")} alt="Xem thêm" />
                                </button>
                            </div>
                        </div>
                    </>
